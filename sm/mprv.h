...
int copy_from_sm(uintptr_t dst, void *src_buf, size_t len)
{
    ...
    if (src % REGBYTES  == 0 && dst % REGBYTES == 0) {
        while (len >= MPRV_BLOCK) {
            int res = copy_block_from_sm(dst, (mprv_block_t *)src);
            if (res)
                return res;
...
int copy_to_sm(void *dst_buf, uintptr_t src, size_t len)
{
    ...
    if (src % REGBYTES == 0 && dst % REGBYTES == 0) {
        while (len >= MPRV_BLOCK) {
            int res = copy_block_to_sm((mprv_block_t *)dst, src);
            if (res)
                return res;
